"""Paper-ready plots for covariate + concept shift experiments (delta_x sweep).

This script consumes evaluation artifacts produced by
`notebook/eval_cov_and_concept_shift.py` (expects an `eval-root` that
contains a `summaries/` subfolder).
...
Outputs are stored beneath `eval_out/final_paper_ready/cov_and_concept_shift/`.
